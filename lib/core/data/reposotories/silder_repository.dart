...
class SliderRepository {
  Future<Either<String, List<SilderModel>>> getAllSliders() async {
    try {
      return NetworkUtil.sendRequest(
        requestType: RequestType.GET,
        url: SliderEndpoints.allSliders,
        ...
        (response) {
          ...
          if (commonResponse.getStatus) {
            List<SilderModel> resultList = [];

            commonResponse.data!["sliders"].forEach(
              (element) {
                resultList.add(SilderModel.fromJson(element));
              },
            );
